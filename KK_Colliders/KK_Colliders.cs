...
namespace KK_Colliders
{
    ...
    public class KK_Colliders : BaseUnityPlugin
    {
        ...
        private static void SetUpColliders(ChaControl chaControl)
        {
            var controller = GetController(chaControl);
            if (controller != null && controller.ChaDidColliders)
                return;
            ...
            if (BreastColliders.Value)
                foreach (var x in chaControl?.gameObject?.GetComponentsInChildren<Transform>(true))
                    if (x.name == "p_cf_body_bone")
                        TweakBody(chaControl, x.gameObject);
            ...
            if (SkirtColliders.Value)
                foreach (DynamicBone x in chaControl?.gameObject?.GetComponentsInChildren<DynamicBone>(true))
                    if (x.name == "ct_clothesBot")
                        TweakSkirt(chaControl, x);

            if (controller != null)
                controller.ChaDidColliders = true;
            else
                DidColliders = true;
        }

        public static void TweakSkirt(ChaControl ctrl, DynamicBone db)
        {
        ...
        public static DynamicBoneCollider AddCollider(GameObject go, string bone, float sx = 1, float sy = 1, float sz = 1, Vector3 r = new Vector3(), Vector3 t = new Vector3())
        {
        ...
        public static void Reload(ChaControl __instance) => SetUpColliders(__instance);
        ...
        {
            internal bool ChaDidColliders { get; set; } = false;
            ...
            protected override void OnReload(GameMode currentGameMode)
            {
                if (DidColliders)
                {
                    ChaDidColliders = true;
                    DidColliders = false;
                }
                SetUpColliders(ChaControl);
            }
        }
    }
}